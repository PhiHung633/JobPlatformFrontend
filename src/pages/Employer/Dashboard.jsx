...
import { fetchApplicationsByJob, fetchApplicationsPerMonth, fetchUserById, processMomoPayment } from '../../utils/ApiFunctions';
...
const Dashboard = () => {
    ...
    const [applicationsPerMonthData, setApplicationsPerMonthData] = useState([]);
    const [applicationsByJobData, setApplicationsByJobData] = useState([]);
    ...
    const loadApplicationsPerMonth = async (uId, month, year) => {
        const { data, error: applicationsMonthError } = await fetchApplicationsPerMonth(uId, month, year);
        if (data) {
            setApplicationsPerMonthData(data);
        } else {
            console.error("Error fetching applications per month data:", applicationsMonthError);
            setError("Failed to fetch applications per month data.");
        }
    };
        ...
        const fetchAllDashboardData = async () => {
            ...
            if (token) {
                try {
                    ...
                    const { data: userDataResponse, error: userError } = await fetchUserById(currentUserId);
                    if (userDataResponse) {
                    ...
                    await loadApplicationsPerMonth(currentUserId, selectedMonth, selectedYear);


                    const { data: applicationsJobData, error: applicationsJobError } = await fetchApplicationsByJob(currentUserId);
                    if (applicationsJobData) {
                        setApplicationsByJobData(applicationsJobData);
                    } else {
                        console.error("Error fetching applications by job data:", applicationsJobError);
                    }
